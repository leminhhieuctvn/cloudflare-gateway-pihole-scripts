...
export const getAccountConfigs = () => {
  ...
  // Add first account (original configuration)
  ...
  // Add additional accounts (2, 3, 4, etc.)
  let accountNumber = 2;
  while (true) {
    const apiToken = process.env[`CLOUDFLARE_API_TOKEN_${accountNumber}`];
    const accountId = process.env[`CLOUDFLARE_ACCOUNT_ID_${accountNumber}`];
