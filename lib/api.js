...
export const deleteZeroTrustListsForAllAccounts = async (accountConfigs) => {
  for (const accountConfig of accountConfigs) {
    try {
      ...
      if (lists.length > 0) {
        console.log(`Found ${lists.length} lists for Account ${accountConfig.accountNumber}, deleting all...`);
        console.log(`List names: ${lists.map(list => list.name).join(', ')}`);
        await deleteZeroTrustListsOneByOne(lists, accountConfig);
      } else {
        console.log(`No lists found for Account ${accountConfig.accountNumber}`);
      }
    } catch (err) {
...
export const deleteZeroTrustRulesForAllAccounts = async (accountConfigs) => {
  for (const accountConfig of accountConfigs) {
    try {
      ...
      if (rules.length > 0) {
        console.log(`Found ${rules.length} rules for Account ${accountConfig.accountNumber}, deleting all...`);
        for (const rule of rules) {
          await deleteZeroTrustRule(rule.id, accountConfig);
        }
      } else {
        console.log(`No rules found for Account ${accountConfig.accountNumber}`);
      }
    } catch (err) {
